...
function baseResponse(originalUrl, normalized) {
    return {
...
async function fetchOEmbed(normalized, appToken, originalUrl) {
    ...
    try {
        const response = await fetch(`https://graph.facebook.com/${GRAPH_VERSION}/${endpointFor(normalized.provider)}?${params}`, { signal: controller.signal, headers: { Accept: 'application/json' } });
